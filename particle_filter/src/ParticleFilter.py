...
class ParticleFilter():
  ...
  def initialize_global(self):
    ...
    print ("Inside initialize global")
    ...
    self.particles[:, 0] = np.random.choice(permissible_x, size=self.particles.shape[0])
    self.particles[:, 1] = np.random.choice(permissible_y, size=self.particles.shape[0])
    self.particles[:, 2] = np.random.uniform(-np.pi, np.pi, size=self.particles.shape[0])
  ...
  def expected_pose(self):
    ...
    print ("Inide expected pose") 
    # Extract x, y, and theta (yaw) components of particles
    particle_x = self.particles[:, 0]
    particle_y = self.particles[:, 1]
    particle_theta = self.particles[:, 2]
    # Compute weighted average of x, y, and theta using particle weights
    #avg_x = np.average(particle_x, weights=self.weights)
    #avg_y = np.average(particle_y, weights=self.weights)
    avg_x = np.sum(particle_x * self.weights)/np.sum(self.weights)
    avg_y = np.sum(particle_y * self.weights)/np.sum(self.weights)

    # Compute weighted average of sine and cosine of the yaw (theta)
    avg_cos_theta = np.sum(np.cos(particle_theta)*self.weights)
    avg_sin_theta = np.sum(np.sin(particle_theta)*self.weights)

    # Compute the weighted average theta using arctan2
    avg_theta = np.arctan2(avg_sin_theta, avg_cos_theta)
  ...
  def publish_particles(self, particles):
    print ("Inside publish particles")
    pa = PoseArray()
