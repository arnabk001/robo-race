...
KM_DELTA_NOISE = 0.3 # Kinematic car delta noise std dev
...
class KinematicMotionModel:
  ...
  def motion_cb(self, msg):
    ...
    v_noise = np.random.normal(0, KM_V_NOISE, self.particles.shape[0])
    delta_noise = np.random.normal(0, KM_DELTA_NOISE, self.particles.shape[0])

    # Make sure different control noise is sampled for each particle
    v_noisy = curr_speed + v_noise
    delta_noisy = curr_steer_angle + delta_noise

    # Propagate particles through kinematic model with noisy controls
    self.particles[:, 0] += v_noisy * np.cos(self.particles[:, 2]) * dt
    self.particles[:, 1] += v_noisy * np.sin(self.particles[:, 2]) * dt
    self.particles[:, 2] += (v_noisy / self.CAR_LENGTH) * np.tan(delta_noisy) * dt

    # Sample model noise for each particle
    x_noise = np.random.normal(0, KM_X_FIX_NOISE, self.particles.shape[0])
    y_noise = np.random.normal(0, KM_Y_FIX_NOISE, self.particles.shape[0])
    theta_noise = np.random.normal(0, KM_THETA_FIX_NOISE, self.particles.shape[0])

    # Add model noise to particles
    self.particles[:, 0] += x_noise
    self.particles[:, 1] += y_noise
    self.particles[:, 2] += theta_noise

    # Limit particle theta to be between -pi and pi
    #self.particles[:, 2] = np.clip(self.particles[:, 2], -np.pi, np.pi)
    '''
    # YOUR CODE HERE
    N = len(self.particles)
    
      # update heading by adding random control noise sampled for N partles to steering angle
    self.particles[:, 2] += curr_steer_angle + ( np.random.normal(size=N) * KM_DELTA_NOISE)
  
      # propagate particles in model direction with noisy controls
    dist = (curr_speed * dt) + (np.random.normal(size=N) * KM_V_NOISE)

      # add model noise for each particle
    self.particles[:, 0] += np.cos(self.particles[:, 2]) * dist + (np.random.normal(size=N) * KM_X_FIX_NOISE)
    self.particles[:, 1] += np.sin(self.particles[:, 2]) * dist + (np.random.normal(size=N) * KM_Y_FIX_NOISE)
    self.particles[:, 2] +=  np.random.normal(size=N) * KM_THETA_FIX_NOISE
    # Limit particle theta to be between -pi and pi
